...
// Mirror office/create.ts so the two stay in sync. Edits here should be mirrored there.
...
const command: Command = {
  ...
  async execute(interaction: ChatInputCommandInteraction): Promise<void> {
    ...
    // Resolve office by name (ilike, exact-first then partial)
    ...
    switch (field) {
      ...
      case 'permissions': {
        ...
        // Replace the array entirely (not append).
      ...
      case 'discord-role': {
        // Accept a role mention like <@&123>, a raw snowflake, or "none"/"clear".
